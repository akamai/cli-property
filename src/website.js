...
class WebSite {
  ...
  _init() {
    ...
    return this.retrieveFormats(true).then(format => {
  ...
  _getCloneConfig(srcProperty, srcVersion = LATEST_VERSION.LATEST) {
    ...
    return this._getProperty(srcProperty, srcVersion).then(cloneFromProperty => {
      contractId = cloneFromProperty.contractId;
      groupId = cloneFromProperty.groupId;

      const productionHosts = cloneFromProperty.productionHosts;
      const stagingHosts = cloneFromProperty.stagingHosts;
      const latestHosts = cloneFromProperty.latestHosts;

      const hosts = productionHosts || stagingHosts || latestHosts;
      if (hosts) {
  ...
  _getHostnameList(propertyId, versionLookup = 0, newConfig = false, edgeHostnameId = null) {
    ...
    return this._getProperty(propertyId).then(property => {
      const version = WebSite._getLatestVersion(property, versionLookup);
  ...
  _findProperty(propertyLookup) {
    const searchObj = {'propertyName': propertyLookup};
    return this._searchByValue(searchObj).then(data => {
      ...
      if (data && data.versions && data.versions.items.length > 0) {
        ...
        const searchObj = {'hostname': propertyLookup};
      ...
      if (data && data.versions.items.length > 0) {
        ...
        const searchObj = {'edgeHostname': propertyLookup};
        return this._searchByValue(searchObj);
      }
    }).then(data => {
  ...
  _getProperty(propertyLookup, hostnameEnvironment = LATEST_VERSION.STAGING) {
    ...
    return this._init().then(() => {
      ...
      if (!prop) {
        const host = this._propertyByHost[propertyLookup];
        if (host)
  ...
  _getPropertyRules(propertyLookup, version, fallThrough = false) {
    return this._getProperty(propertyLookup).then((data) => {
      // set basic data like contract & group
      ...
      return new Promise((resolve, reject) => {
        ...
        this._edge.send(function(data, response) {
          if (/application\/json/.test(response.headers['content-type'])) {
            const parsed = JSON.parse(response.body);
            const matches = !parsed.versionLink ? null : parsed.versionLink.match('versions/(\\d+)?');
            if (!matches) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._edge.send(function(data, response) {
        if (response.statusCode >= 200 && response.statusCode < 400) {
          const propertyResponse = JSON.parse(response.body);
  ...
  _updatePropertyBehaviors(rules, configName, hostname, cpcode, origin = null, secure = false) {
    return new Promise((resolve, reject) => {
      ...
      if (!cpCodeExists) {
        const behavior = {'options': {'value': {'id': Number(cpcode)}}};
        behaviors.push(behavior);
      }
  ...
  _updatePropertyRules(propertyLookup, version, rules) {
    return this._getProperty(propertyLookup).then((data) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        this._edge.send(function(data, response) {
          if (response.statusCode >= 200 && response.statusCode < 400) {
            const newRules = JSON.parse(response.body);
            resolve(newRules);
          } else {
  ...
  _createCPCode(groupId, contractId, productId, configName) {
    return new Promise((resolve, reject) => {
      ...
      this._edge.send((data, response) => {
        if (response.statusCode >= 200 && response.statusCode < 400) {
          const parsed = JSON.parse(response.body);
          const cpcode = parsed['cpcodeLink'].split('?')[0].split('/')[4].split('_')[1];
          resolve(cpcode);
        } else {
  ...
  _createEdgeHostname(groupId, contractId, configName, productId, edgeHostnameId = null, edgeHostname = null, force = false, secure = false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._edge.send((data, response) => {
        if (response.statusCode >= 200 && response.statusCode < 400) {
          const hostnameResponse = JSON.parse(response.body);
    ...
    return this._getProperty(propertyLookup).then((data) => {
      ...
      // TODO what about errors?
        return new Promise((resolve, reject) => {
          ...
          const matches = !body.activationLink ? null : body.activationLink.match('activations/([a-z0-9_]+)\\b');
    ...
    return this._getProperty(propertyLookup).then((data) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        this._edge.send(function(data, response) {
          ...
          if (response.statusCode >= 200 && response.statusCode <= 400) {
            const parsed = JSON.parse(response.body);
            const matches = !parsed.activationLink ? null : parsed.activationLink.match('activations/([a-z0-9_]+)\\b');
  ...
  _assignHostnames(groupId, contractId, configName, edgeHostnameId, propertyId, hostnames, deleteHosts = null, newConfig = false) {
    ...
    return this._getHostnameList(configName, version, false, edgeHostnameId).then(hostnamelist => {
      ...
      return new Promise((resolve, reject) => {
        ...
        const hostSet = new Set(hostsToProcess);
        const hostNamelist = [];
  ...
  static mergeAdvancedUUIDRules(oldRules, newRules) {
    ...
    const search = (ruleNode, parentRules = [], found = {}) => {
      const nodeList = ruleNode.behaviors.concat(ruleNode.criteria);
      nodeList.forEach(advNode => {
        // look for "advanced" behaviors
        if (advNode && (advNode.name === 'advanced' ||
          advNode.name === 'matchAdvanced')) {
          const xml = advNode.options.xml || '' +
            advNode.options.openXml || '' +
            advNode.options.closeXml || '';
          const newParentRules = ruleNode.uuid !== 'default' ? parentRules.concat([ruleNode]) : parentRules;
          const foundNode = {
            uuid: advNode.uuid,
            xml: xml,
            advNode: advNode,
    ...
    const oldAdvMtdBehaviors = search(oldRules);
    const newAdvMtdBehaviors = search(newRules);
    Object.keys(newAdvMtdBehaviors).forEach(key => {
      newAdvMtdBehaviors[key].forEach(newAdvObject => {
        const oldAdvObjectList = oldAdvMtdBehaviors[key] || [];
        const oldAdvObject = oldAdvObjectList.find(x => newAdvObject.parentRules.length === x.parentRules.length);
  ...
  searchProperties(searchString, options) {
    const searchObj = {'propertyName': searchString};
    return this._searchByValue(searchObj).then(result => {
      return result;
    });
  }

  listProperties(groupId, contractId) {
    return this._listProperties(groupId, contractId).then(result => {
      return result;
    });
  }
  ...
  retrieveGroups() {
    return this._getGroupList().then(result => {
  ...
  retrieveFormats(latest = false) {
    let latestRule;
    return this._retrieveFormats().then(result => {
      if (!latest) {
        return Promise.resolve(result.ruleFormats.items);
      } else {
        const items = result.ruleFormats.items.sort();
        items.reverse();
  ...
  retrieve(propertyLookup, versionLookup = LATEST_VERSION.LATEST, hostnames = false) {
    let propertyId;
    return this._getProperty(propertyLookup).then(property => {
      if (!hostnames) {
        const version = (versionLookup && versionLookup > 0) ? versionLookup : WebSite._getLatestVersion(property, versionLookup);
  ...
  retrieveToFile(propertyLookup, toFile, versionLookup = LATEST_VERSION.LATEST) {
    return this.retrieve(propertyLookup, versionLookup).then(data => {
  ...
  update(propertyLookup, newRules) {
    ...
    return this._getProperty(propertyLookup).then(localProp => {
      property = localProp;
      const propertyName = localProp.propertyName;
  ...
  copy(fromProperty, fromVersion = LATEST_VERSION.LATEST, toProperty) {
    return this.retrieve(fromProperty, fromVersion).then(fromRules => {
  ...
  deactivate(propertyLookup, networkEnv = AKAMAI_ENV.STAGING, notes = '', email = ['test@example.com'], wait = true) {
    ...
    return this._getProperty(propertyLookup).then(data => {
      ...
      const deactivationVersion = WebSite._getLatestVersion(property, networkEnv == AKAMAI_ENV.STAGING ? LATEST_VERSION.STAGING : LATEST_VERSION.PRODUCTION) || 1;
  ...
  setCpcode(propertyLookup, version, cpcode) {
    return this._getProperty(propertyLookup).then(data => {
      ...
      if (!cpCodeExists) {
        const behavior = {'name': 'cpCode', 'options': {'value': {'id': Number(cpcode)}}};
        behaviors.push(behavior);
      }
  ...
  delHostnames(propertyLookup, version, hostnames) {
    ...
    const names = this._getConfigAndHostname(propertyLookup, hostnames);
    configName = names[0];
    hostnames = names[1];
  ...
  addHostnames(propertyLookup, version = 0, hostnames, edgeHostname = null) {
    ...
    return this._getProperty(configName).then(data => {
      version = WebSite._getLatestVersion(data, version);
  ...
  setVariables(propertyLookup, version = 0, variablefile) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const newVars = data.rules.variables || [];

      changeVars['create'].map(variable => {
        const indexCheck = newVars.findIndex(elt => elt.name == variable.name);

        if (indexCheck < 0) {
          delete variable.action;
          newVars.push(variable);
          changeVars['update'].splice(changeVars['update'].findIndex(elt => elt.name === variable.name));
        } else {
      ...
      changeVars['delete'].map(variable => {
        newVars.splice(newVars.findIndex(elt => elt.name === variable.name));
        console.error('... deleting variable ' + variable.name);
      });

      changeVars['update'].map(variable => {
        const ind = newVars.findIndex(elt => elt.name == variable.name);
        if (ind >= 0) {
          delete variable.action;
  ...
  create(hostnames = [], cpcode = null, configName = null, contractId = null, groupId = null, newRules = null, origin = null, edgeHostname = null, secure = false, productId = null) {
    ...
    let newEdgeHostname, propertyId, edgeHostnameId;
    ...
    const names = this._getConfigAndHostname(configName, hostnames);
    configName = names[0];
    hostnames = names[1];
    ...
    return this._getPropertyInfo(contractId, groupId, productId).then(data => {
      groupId = data.groupId;
      contractId = data.contractId;
      ...
      const propInfo = data.properties.items[0];
  ...
  createFromExisting(configName, options) {
    ...
    return this._getProperty(srcProperty).then(data => {
      ...
      if ((cloneEhn.hostnames.items) && (!edgeHostnameId)) {
        edgeHostnameId = cloneEhn.hostnames.items[0].cnameTo || cloneEhn.hostnames.items[0].edgeHostnameId;
      }
      return Promise.resolve(edgeHostnameId);
    }).
      then(data => {
